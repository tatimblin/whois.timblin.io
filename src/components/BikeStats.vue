  ...
  <div class="BikeStats">
    <div class="BikeStats-content">
      <div class="BikeStats-header">
        ...
        <span class="BikeStats-date BikeStats-unit Text--quiet" :class="`BikeStats-date--${statusColor}`">
          {{ relativeDate }}
        </span>
      </div>
      <div class="BikeStats-items">
        ...
        <div class="BikeStats-item">
          <span class="BikeStats-stat Heading--sub">{{ roundDown(mostRecent.duration  / 60 / 60)}}</span>
...
const parsed = Papa.parse(bikeStatsRaw, { header: true });

export default {
  name: 'BikeStats',
  data() {
    return {
      mostRecent: parsed.data[0],
    };
  },
  computed: {
    diffDays() {
